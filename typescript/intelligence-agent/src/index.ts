// Intelligence Agent — TypeScript / Fastify v5 — port 8011 (§12.2).
// Implements the full A2A endpoint set (§8), auth (§1), structured JSON
// logging (§16) and SSE streaming (§9). Streams the scout phases on
// /message:stream and runs them synchronously on /message:send.
...
import {
  COMPLETED_PHASE,
  SCOUT_PHASES,
  buildIntelligenceArtifact,
  extractSystem,
} from "./missionLogic.js";
...
//
// Canonical JSON = JSON.stringify with recursively sorted object keys, so the
// serialization (and thus the ETag) is deterministic across implementations.
...
app.post("/message::send", async (request, reply) => {
  ...
  const system = extractSystem(requestMessage);
  ...
  // Run the scout synchronously (no delays) and return the completed Task (§5).
  runScoutSync(task.id, system);
...
app.post("/message::stream", async (request, reply) => {
  ...
  await runScoutStream(reply, task.id, system);
  reply.raw.end();
});
...
function handleCancel(
  ...
  const status: TaskStatus = {
    ...
    timestamp: nowIso(),
    metadata: { phase: "canceled", display: "Scout mission canceled." },
  };
...
/** Run all scout phases with no delay; mutate the task to COMPLETED. */
function runScoutSync(taskId: string, system: string): void {
  ...
  finishScout(taskId, system);
}

/** Stream all scout phases over SSE, then finalize the task (§9 ordering). */
async function runScoutStream(
  reply: FastifyReply,
  taskId: string,
  system: string,
): Promise<void> {
  ...
  const artifact = buildIntelligenceArtifact(system);
  ...
  store.appendHistory(
    ...
    buildAgentMessage(
      task.contextId,
      AGENT_NAME,
      `Intelligence report ready for the ${system} system.`,
    ),
  );
  ...
  log({ event: "scout_completed", taskId, contextId: task.contextId, system });
}

/** Attach the artifact + completed status synchronously (no SSE). */
function finishScout(taskId: string, system: string): void {
  const task = store.get(taskId);
  if (!task) return;
  const artifact = buildIntelligenceArtifact(system);
  store.addArtifact(taskId, artifact);
  ...
  log({ event: "scout_completed_sync", taskId, contextId: task.contextId, system });
}
